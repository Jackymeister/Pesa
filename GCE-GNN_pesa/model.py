...
def train_test(model, train_data, test_data, class_weights):
    ...
    result.append(np.mean(occur) * 100)  # coverage
    result.append((np.sum(np.multiply(class_weights_tail, occur)) / I_t) * 100)  # tail coverage
    result.append(np.mean(tail) * 100)

    result.append(np.mean(hit_f) * 100)
    result.append(np.mean(mrr_f) * 100)

    result.append(np.mean(hit_r) * 100)
    result.append(np.mean(mrr_r) * 100)
